...
int main() {
	...
	// Push another scope so most memory should be freed *before* we exit the app
	{
		...
		shader->SetUniform("u_Option1", (int)option4);
		...
		BackendHandler::imGuiCallbacks.push_back([&]() {
			...
			if (ImGui::Checkbox("Specular and Ambient and Diffuse", &option4))
			{
				option1 = false;
				option2 = false;
				option3 = false;
			...
			if (ImGui::Checkbox("Specular and custom effect (didn't do)", &option5))
			{
				option1 = false;
				option2 = false;
				option3 = false;
		...
		{
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/frog.obj");
