...
int main() {
	...
	// Push another scope so most memory should be freed *before* we exit the app
	{
		...
		// this is a test to see if doing it to all 3 shader types results in it working.
		groundShader->SetUniform("u_LightPos", lightPos);
		groundShader->SetUniform("u_LightCol", lightCol);
		groundShader->SetUniform("u_SpecularLightStrength", lightSpecularPow);
		groundShader->SetUniform("u_AmbientCol", ambientCol);
		groundShader->SetUniform("u_AmbientStrength", ambientPow);
		groundShader->SetUniform("u_LightAttenuationConstant", 1.0f);
		groundShader->SetUniform("u_LightAttenuationLinear", lightLinearFalloff);
		groundShader->SetUniform("u_LightAttenuationQuadratic", lightQuadraticFalloff);
		groundShader->SetUniform("u_Mode", mode);
		groundShader->SetUniform("u_Textures", textures);

		groundShader->SetUniform("s_RampTexture", 1);
		...
		waterShader->SetUniform("u_Mode", mode);
		waterShader->SetUniform("u_Textures", textures);

		waterShader->SetUniform("s_RampTexture", 1);
		...
		theSun._lightCol = glm::vec4(1.1f, 1.1f, 0.6f, 0.0f);
		theSun._ambientCol = glm::vec4(0.5f, 0.5f, 0.5f, 0.0f);
		theSun._lightDirection = glm::vec4(10.f, 10.f, -5.f, 0.0f);
		theSun._shadowBias = 0.005;
	...
	/*	Texture2D::sptr stone = Texture2D::LoadFromFile("images/Stone_001_Diffuse.png");
		...
		Texture2D::sptr stone = Texture2D::LoadFromFile("images/ratTex.png");
		Texture2D::sptr stoneSpec = Texture2D::LoadFromFile("images/ratTex.png");
		...
		GameObject obj3 = scene->CreateEntity("monkey_quads");
		{
			...
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/rat_single_uv1 frame1.obj");
